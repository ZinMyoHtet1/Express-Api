...
module.exports = {
  getAllPhones: asyncErrorHandler(async (req, res, next) => {
    ...
    const feature = model.filter().sort().selectFields().paginate();
  ...
  postNewPhone: asyncErrorHandler(async (req, res, next) => {
    const phone = await Phone.create(
      Object.assign(req.body, { createdBy: req.user._id })
    );
  ...
  getPhoneById: asyncErrorHandler(async (req, res, next) => {
  ...
  updatePhoneById: asyncErrorHandler(async (req, res, next) => {
    const id = req.params.id;
  ...
  deletePhoneById: asyncErrorHandler(async (req, res, next) => {
    const id = req.params.id;
    if (!mongoose.isValidObjectId(id)) {
      const error = new CustomError("Invalid phone Id format", 400);
      return next(error);
